...
Commands:
  ...
  deploy-preflight               Check whether hosted production deploy is wired from this repo
...
cmd_deploy_preflight() {
  ...
  for candidate in \
    ...
    "$ROOT_DIR/wrangler.json" \
    "$ROOT_DIR/wrangler.jsonc" \
    "$ROOT_DIR/package.json" \
    "$ROOT_DIR/render.yaml" \
    "$ROOT_DIR/fly.toml" \
    "$ROOT_DIR/Procfile"
  do
    [[ -f "$candidate" ]] && manifest_count=$((manifest_count + 1))
  done

  echo "hosted_base_url=$BASE_URL"
  echo "workspace_root=$ROOT_DIR"
  echo "git_remote_origin=${remote_url:-none}"
  echo "deploy_manifest_count=$manifest_count"

  if [[ "$remote_url" == *"my-openclaw-root.git"* ]]; then
    echo "blocker=workspace_remote_is_openclaw_root"
    echo "detail=Task Manager workspace is embedded inside the OpenClaw root repo, not a dedicated deploy repo."
    status_code=1
  fi

  if [[ "$manifest_count" -eq 0 ]]; then
    echo "blocker=no_hosted_deploy_manifest"
    echo "detail=No wrangler/render/fly/procfile/package deployment manifest exists in this workspace for tm.lidisolutions.ai."
    status_code=1
  fi

  if [[ "$status_code" -ne 0 ]]; then
    echo "deploy_ready=false"
    echo "next_action=Create or attach a real production deployment target for tm.lidisolutions.ai, then rerun deploy-preflight."
    return "$status_code"
  fi

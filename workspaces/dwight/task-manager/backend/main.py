...
AGENT_USERS = parse_csv_env(
    os.getenv(
        "AGENT_USERS",
        "Overseer,Researcher,Quant,Critic,Trader,Executor,Archivist,Developer",
    )
)
AGENT_ALLOWED_REPOS = set(
    parse_csv_env(
        os.getenv(
            "AGENT_ALLOWED_REPOS",
            "aarwitz/AutoTap,aarwitz/lidi-task-manager,aarwitz/Task-Manager",
        )
    )
)
AUTO_APPROVE_EMAILS = {email.lower() for email in parse_csv_env(
    os.getenv(
        "AUTO_APPROVE_EMAILS",
        "taylor@lidisolutions.ai,aaronclawrsl@gmail.com",
    )
)}
...
GITHUB_WEBHOOK_TRUSTED_REPOS = set(
    parse_csv_env(
        os.getenv(
            ...
            "aarwitz/AutoTap,aarwitz/Task-Manager,aarwitz/lidi-task-manager",
        )
    )
)
...
CANONICAL_TM_USERS = ["Dwight", "Jerry", "Druck", "Aaron", "Taylor"]
LOGIN_ALLOWED_USERS = CANONICAL_TM_USERS.copy()
INTERNAL_HUMAN_USERS = ["Aaron", "Taylor"]
EXECUTING_AGENT_USERS = ["Dwight", "Jerry", "Druck"]
ALL_AGENT_USERS = []
for value in [*EXECUTING_AGENT_USERS, *AGENT_USERS]:
...
def validate_assignee_identity(db: Session, value: Optional[str], *, field_name: str, allow_blank: bool = False) -> Optional[str]:
    ...
    if canonical in CANONICAL_TM_USERS or canonical in AGENT_USERS:
        return canonical
    ...
    allowed = ", ".join(CANONICAL_TM_USERS + AGENT_USERS)
    raise HTTPException(
        status_code=400,
...
def ensure_issue_is_agent_scoped(issue: models.Issue):
    repo_slug = normalize_optional_text(issue.repo_slug)
    if not repo_slug or repo_slug not in AGENT_ALLOWED_REPOS:
        allowed = ", ".join(sorted(AGENT_ALLOWED_REPOS))
        raise HTTPException(
            status_code=400,
            detail=f"Issue is outside allowed scope. Allowed repo_slug values: {allowed}",
        )
...
def list_public_users_for_admin(request: Request, db: Session = Depends(get_db)):
    ...
    return (
        db.query(models.PublicUser)
        .order_by(models.PublicUser.is_owner.desc())
...
@app.post("/api/public/users/{user_id}/owner", response_model=schemas.PublicUserResponse)
def update_public_user_owner(
    user_id: int,
    payload: schemas.PublicUserOwnerUpdate,
    request: Request,
    db: Session = Depends(get_db),
):
    approver = require_public_session(request, db, owner_only=True)
    ...
    target_owner = bool(payload.is_owner)

    if user.id == approver.id and not target_owner:
        raise HTTPException(status_code=403, detail="You cannot remove owner access from your current account")

    if target_owner:
        if user.status != 'approved':
            mark_public_user_approved(user, approved_by_email=approver.email)
            db.query(models.SignupRequest).filter(
                models.SignupRequest.email == user.email,
                models.SignupRequest.status_snapshot.in_(['pending', 'pending_email_verification']),
            ).update(
        ...
        owner_count = db.query(models.PublicUser).filter(models.PublicUser.is_owner == True).count()
        if user.is_owner and owner_count <= 1:
            raise HTTPException(status_code=403, detail="At least one owner account must remain")

    user.is_owner = target_owner
    user.updated_at = datetime.now()
    db.commit()
    db.refresh(user)
    return user
...
def get_agent_config():
    """Expose automation boundaries for worker scripts and UI integrations."""
    return {
        "agent_users": AGENT_USERS,
        "allowed_repos": sorted(AGENT_ALLOWED_REPOS),
        "scope": "agent-scope",
    }


@app.get("/api/agent/issues", response_model=List[schemas.IssueResponse])
def get_agent_issues(
    status_filter: Optional[str] = Query(None, alias="status"),
    assigned_to: Optional[str] = None,
    sprint_id: Optional[int] = None,
    db: Session = Depends(get_db),
):
    """Return issues restricted to scoped repositories only."""
    query = db.query(models.Issue).filter(models.Issue.repo_slug.in_(AGENT_ALLOWED_REPOS))
...
@app.post("/api/agent/issues/{issue_id}/claim", response_model=schemas.IssueResponse)
def claim_agent_issue(issue_id: int, assigned_to: str = Query(...), db: Session = Depends(get_db)):
    """Assign a scoped issue to an allowed agent user."""
    issue = db.query(models.Issue).filter(models.Issue.id == issue_id).first()
    if not issue:
        raise HTTPException(status_code=404, detail="Issue not found")

    ensure_issue_is_agent_scoped(issue)
    assignee = validate_assignee_identity(db, assigned_to, field_name="assigned_to")
    if assignee not in AGENT_USERS:
        raise HTTPException(status_code=400, detail="Issue claims must assign to an allowed agent user")
...
@app.post("/api/agent/issues/{issue_id}/agent-comment", response_model=schemas.CommentResponse)
def add_agent_comment(issue_id: int, comment: schemas.CommentCreate, db: Session = Depends(get_db)):
    """Post a progress update as an allowed agent on a scoped issue."""
    issue = db.query(models.Issue).filter(models.Issue.id == issue_id).first()
    if not issue:
        raise HTTPException(status_code=404, detail="Issue not found")
    ...
    if actor not in AGENT_USERS:
        raise HTTPException(status_code=400, detail="Only allowed agent users can use this endpoint")
...
def create_lidi_action_draft(payload: schemas.LidiActionDraftCreate, request: Request, db: Session = Depends(get_db)):
    ...
    if action_type == "create_issue":
        ...
        if draft_repo and draft_repo in AGENT_ALLOWED_REPOS and draft_assignee and draft_assignee not in AGENT_USERS:
            raise HTTPException(status_code=400, detail="Scoped issues must be assigned to an allowed agent user")
...
async def check_issue_pr_contract(issue_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    if should_comment:
        ...
        comment = models.Comment(content="\n".join(body), username="TaskFlow-PM", issue_id=issue_id)
        db.add(comment)
        log_issue_activity(db, issue_id, "comment_added", actor="TaskFlow-PM", new_value=(comment.content or "")[:120])
        issue.updated_at = datetime.now()
        db.commit()
...
async def github_webhook(
    ...
    for issue_id in issue_ids:
        ...
        comment = models.Comment(
            content="\n".join(comment_body),
            username="TaskFlow-PM",
            issue_id=issue.id,
        )
        db.add(comment)
        log_issue_activity(db, issue.id, "comment_added", actor="TaskFlow-PM", new_value=(comment.content or "")[:120])
        issue.updated_at = datetime.now()
        updated.append({
            "issue_id": issue.id,
            "status": issue.status,

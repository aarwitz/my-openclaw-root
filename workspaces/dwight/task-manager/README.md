...
scripts/tmctl.sh deploy-preflight
...
- `tmctl deploy-preflight` tells you whether this workspace can actually deploy hosted `tm.lidisolutions.ai`
- `TM_BASE_URL` must remain `https://tm.lidisolutions.ai` (strict hosted-only hardening)
- LAN exposure is controlled by `TM_PUBLISH_HOST` (default `0.0.0.0`) for Dwight-maintained local validation only.

Canonical operations workflow: see `WORKFLOW.md`.

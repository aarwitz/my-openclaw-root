...
Read both `~/.openclaw/state/market-event-brief.json` (market-wide intake,
refreshed every 15 minutes during the session) and
`~/.openclaw/state/catalyst_brief.json` (ticker-level quant/news context) before
sourcing ideas. The market-event brief exists because ticker-first lookup
cannot discover a forced liquidation, portfolio transfer, policy shock, or
crowding unwind before the affected names are known. Publication time,
retrieval time, event class, and flow phase are evidence; the classifier is an
advisory triage label, not a fact or trade signal.

For every market-wide event that appears economically material, either
(a) investigate it against primary sources and connect the plausible affected
names/mechanism, or (b) record a concise reason it is irrelevant/noisy. A
`transfer_complete` forced-flow event is specifically a prompt to test whether
temporary price pressure has ended; it is never permission to blindly buy.

The ticker catalyst brief fuses quant conviction, news catalysts, social
attention, and the nightly cross-sectional GBM rank per name.
...
- Before researching, read `~/.openclaw/state/theme-context.json` (generated by
  the deterministic pass). For every beneficiary/victim overlap, either link
  the canonical `theme_id` in `hypotheses.theme_id` and address why the name is
  on that side of the theme, or state in the research journal why the theme is
  irrelevant. A theme is retrieval context, never sufficient evidence or trade
  authority by itself.

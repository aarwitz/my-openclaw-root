...
- `trade_intent.state`: `proposed` → `critic_review` → `risk_review` → `approved` | `blocked` → `submitted` → `filled` | `partial` | `canceled` | `rejected`.
- `order.state`: owned simulator order status, linked to `trade_intent` and `position`.
...
2. The latest review is authored by `critic`, has at least two concrete,
   developed challenges/responses, and every challenge is resolved.
...
6. Vehicle is supported by the internal simulator (equity/ETF only).
...
## 6. Unsupported vehicles

Options, assignment, margin, and leveraged-product intents fail closed. Adding a
vehicle requires simulator semantics, schema support, money-path regression
tests, and an operating-authority update.

## 7. Reconciliation

- Executor verifies intent → simulator order → fill → position/tranche lineage
  and cash/position arithmetic every checkpoint.
- A filled simulator order without the expected canonical lineage creates a
  `reconciliation_run` and pauses new opens for the affected hypothesis.
- On ledger-wide anomalies or unresolved divergence, the system enters degraded
  mode via `system_pauses.scope = exits_trims_only` until cleared.
...
These cases must pass before the internal-paper system is considered production-ready:
...
6. Unsupported vehicle intent is rejected.
